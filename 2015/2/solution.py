...
def get_paper_order(data):
    ...
    for line in lines:
        ...
        paper_order += smallest_side + sum(side*2 for side in sides)
    return paper_order


def get_ribbon_order(data):
    lines = data.split("\n")
    ribbon_order = 0
    for line in lines:
        dims = list(map(int, line.split("x")))
        dims.sort()
        ribbon_order += sum(dim + dim for dim in dims[:-1])
        accu = dims[0]
        for dim in dims[1:]:
            accu *= dim
        ribbon_order += accu
...
if __name__ == "__main__":
    ...
    print(get_paper_order(data=instructions))
    print(get_ribbon_order(data=instructions))
